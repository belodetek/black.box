...
def run_openvpn_mgmt_cmd(host=VPN_HOST, port=VPN_UDP_MGMT_PORT, cmd='status 2'):
	...
	try:
		for res in socket.getaddrinfo(host, port, socket.AF_UNSPEC, socket.SOCK_STREAM):
			...
			if DEBUG: log('getaddrinfo: res={} af={} socktype={} proto={} canonname={} sa={}'.format(
				res,
				af,
				socktype,
				proto,
			...
			try:
				...
				log('socket: msg={} af={} socktype={} proto={} canonname={} sa={}'.format(
					msg,
					af,
					socktype,
					proto,
		...
		if DEBUG: print('data: {}', format(data))
		s.sendall(b'quit\n')
		s.close()
	except:
		pass
...
def connect_node(family=AF):
	...
	if DEVICE_TYPE == 5 or (
		...
		log('{}: mode={} provider={} group={} location={} username={} password={}'.format(
			stack()[0][3],
			DEVICE_TYPE,
			VPN_PROVIDER,
			VPN_LOCATION_GROUP,
		...
		if PAIRED_DEVICE_GUID:
			try:
				...
				log('get_node_by_guid: ipaddr={} af={} guid={}'.format(
					ipaddr,
					family,
					PAIRED_DEVICE_GUID
				))
				...
				try:
					...
					if DEBUG: print_exc()
					log('get_node_by_guid: e={} af={}'.format(
						repr(e),
						family
					))
		...
		###################
			...
			if family == 6 and not SKIP_DNS:
				try:
					...
					log('resolve_dns: e={} qtype={} family={}'.format(
						repr(e),
						qtype,
						family
					))
			...
			log('{}: qtype={} country={} host={} ipaddr={}'.format(
				stack()[0][3],
				qtype,
				country,
				host,
			...
			if not ipaddr:
				if family == 6:
					try:
						...
						log('get_node_by_country: ipaddr={} af={}'.format(
							stack()[0][3],
							ipaddr,
							family
						))
					except AssertionError as e:
						log('get_node_by_country: e={} af={}'.format(
							repr(e),
							family
						))
						if DEBUG: print_exc()
		...
		log('{}: mode={} remote={} port={} proto={} protos={} cipher={} auth={}'.format(
			stack()[0][3],
			DEVICE_TYPE,
			ipaddr,
			port,
		...
		if STUNNEL:
			...
			if result[0] == 0:
				...
				log('{}: stunnel={} proto={} ipaddr={}'.format(
					stack()[0][3],
					STUNNEL,
					tun_proto,
					ipaddr
	...
	log('{}: ipaddr={} af={} qtype={} proto={} port={} country={} guid={} provider={} group={} location={} username={}'.format(
		stack()[0][3],
		ipaddr,
		family,
		qtype,
...
def _get_client_conns(user=GUID, proto='udp'):
	conns = 0
	stats = open('{}/openvpn.{}.status'.format(
		DATADIR,
		proto
	)).read().split('\n')
	for lines in stats:
...
def get_status(user=GUID, proto='udp'):
	status = list()
	status = open('{}/openvpn.{}.status'.format(
		DATADIR,
		proto
	)).read().split('\n')
	return status
...
def reauthenticate_clients():
	...
	while True:
		...
		for client in clients:
			...
			print('get_device_env_by_name: client={} password={}'.format(
				client,
				password
			))
			if not result and client != 'UNDEF':
				for port in [VPN_UDP_MGMT_PORT, VPN_TCP_MGMT_PORT]:
					try:
						print('run_openvpn_mgmt_cmd: port={} cmd=kill client={} result={}'.format(
							port,
							client,
							result
						))
						print(run_openvpn_mgmt_cmd(
							port=port,
							cmd='kill {}'.format(client)
						))
						killed.append(client)
					except:
						pass
		print('{}: count={} killed={}'.format(
			stack()[0][3],
			count,
			len(killed)
		))
...
def disconnect_clients():
	...
	while True:
		if os.path.exists('{}/disconnect_clients'.format(DATADIR)):
			...
			for client in clients:
				if client not in ['UNDEF']:
					try:
						...
						print('client_disconnect: client={} result={}'.format(
							client,
							result
						))
					except:
			...
			print('{}: plugin={} disconnected={}'.format(
				stack()[0][3],
				DNS_SUB_DOMAIN,
				len(disconnected)
			))
